...
class SpreadsheetQuotationCreate(models.TransientModel):
    ...
    def _build_cells(self, list_id, columns, line_count):
        # Odoo 19's o-spreadsheet stores unversioned data as
        # {xc: "content string"} directly, not {xc: {"content": ...}} as in
        # 18.0. Without a "version" key, the legacy per-cell object format
        # is fed straight into the plugins and crashes (content.startsWith
        # is not a function) instead of going through the migration system.
        cells = {}
        for col_idx, col_name in enumerate(columns):
            col_letter = self._col_index_to_letter(col_idx)
            cells[f"{col_letter}1"] = f'=ODOO.LIST.HEADER({list_id},"{col_name}")'
            for row in range(1, line_count + 1):
                cells[f"{col_letter}{row + 1}"] = (
                    f'=ODOO.LIST({list_id},{row},"{col_name}")'
                )
        return cells
