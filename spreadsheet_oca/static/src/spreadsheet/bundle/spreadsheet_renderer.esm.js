...
class SpreadsheetTransportService {
    ...
    async sendMessage(message) {
        let isAccepted;
        try {
            isAccepted = await this.orm.call(this.model, "send_spreadsheet_message", [
                [this.res_id],
                message,
            ]);
        } catch (error) {
            // The o-spreadsheet collaborative session debounces cursor-move
            // messages by 200ms and doesn't cancel that timer on
            // leaveSession(), so a move can still be in flight after the
            // owning component (and its protected `orm` service) is
            // destroyed. That's an expected race, not a real failure.
