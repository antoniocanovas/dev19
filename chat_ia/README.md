...
- **Verificado por ORM** (usuarios y canales de prueba creados y luego revertidos con
  ...
  - El usuario del bot no puede autenticarse (`_check_credentials` lanza `AccessDenied` pase lo
    que pase).
  - `base.user_admin` pertenece a `chat_ia.group_ai_chat_user` (seed de
    `security/chat_ia_security.xml` confirmado en base real).
...
| Módulo | Para qué |
|---|---|
| `odoo_mcp_manager` | Aporta `ai.tool`/`ai.provider` (el `chat.ia.responder` por defecto llama a `ask_ai`) y `ai.bot.conversation`, reutilizado aquí para persistir el historial de cada canal de Discuss. |
| `mail` | `discuss.channel`, el hook `_message_post_after_hook`. |

## Qué instala

### 1. El usuario del bot (`data/chat_ia_bot_data.xml`)

Un `res.users` real (no solo un `res.partner`) llamado "Asistente IA", `login=chat_ia_bot`. Es un
...
### 2. El grupo "Conversaciones IA" (`security/chat_ia_security.xml`)

`chat_ia.group_ai_chat_user` — quien no pertenezca a él recibe siempre el mensaje fijo *"No
...
### 3. El enganche con Discuss (`models/discuss_channel.py`, `models/chat_ia_bot.py`)
...
cualquier canal, `chat.ia.bot._apply_logic()`:
...
3. Comprueba el grupo `chat_ia.group_ai_chat_user` del autor real del mensaje (`self.env.user`,
...
4. Si pertenece, delega en `chat.ia.responder._get_reply(text, history)` (ver siguiente punto) y
...
### 4. `chat.ia.responder` — el punto de extensión para la lógica de negocio

`models/chat_ia_responder.py` define un `models.AbstractModel` con un único método,
`_get_reply(text, history)`, y una implementación por defecto: un passthrough sencillo a la tool
genérica `ask_ai` de `odoo_mcp_manager`, sin ninguna restricción de dominio. `chat_ia` no asume
qué módulo de negocio (si alguno) está instalado.

Los módulos de negocio deben hacer `_inherit = 'chat.ia.responder'` y sobreescribir
`_get_reply` con su propia clasificación/tools — ver `mercas_ai/models/chat_ia_responder.py`,
...
aparece solo en su barra lateral de Discuss, sin acción manual. Un campo `chat_ia_greeted`
...
- Este módulo no incluye ninguna lógica de negocio propia — instalarlo solo (sin `mercas_ai`
  u otro módulo que sobreescriba `chat.ia.responder`) deja el bot respondiendo con un `ask_ai`
