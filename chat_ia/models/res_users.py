...
class ResUsers(models.Model):
    ...
    chat_ia_greeted = fields.Boolean(
        default=False,
        readonly=True,
        help="Whether the Chat IA bot has already sent this user its "
             "welcome DM. Prevents re-greeting on every page load.",
    )

    def _check_credentials(self, credential, env):
        """Hard-block any login attempt for the Chat IA bot user, no matter
    ...
    def _on_webclient_bootstrap(self):
        ...
        try:
            self._chat_ia_maybe_greet()
        except Exception:
            _logger.exception("chat_ia: greeting failed for user %s", self.id)

    def _chat_ia_maybe_greet(self):
        ...
        self.ensure_one()
        if self.chat_ia_greeted or not self._is_internal():
            return
        if not self.has_group("chat_ia.group_ai_chat_user"):
            return
        bot = self.env.ref("chat_ia.user_chat_ia_bot", raise_if_not_found=False)
        ...
        self.sudo().chat_ia_greeted = True
