...
class AiDomainChatWizard(models.TransientModel):
    """Single console covering ventas/compras/facturación/stock.

    One fast LLM call classifies the question into a domain and extracts
    query parameters; anything outside those four domains gets a fixed
    refusal with no further tool call. Each domain is answered by a
    deterministic read_group tool (see models/ai_tool.py), so figures are
    always exact -- the LLM never has to compute totals itself.
    """

    _name = 'mercas.mcp.domain.chat.wizard'
    _description = 'Consultas IA (Ventas / Compras / Facturación / Stock)'

    message = fields.Text(string='Pregunta')
